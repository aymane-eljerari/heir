...
CiphertextT dot_product__encrypt__arg0(CryptoContextT cc, std::vector<float> v0, PublicKeyT pk) {
...
CiphertextT dot_product__encrypt__arg1(CryptoContextT cc, std::vector<float> v0, PublicKeyT pk) {
  std::vector<double> v1(std::begin(v0), std::end(v0));
  auto v1_filled_n = cc->GetCryptoParameters()->GetElementParams()->GetRingDimension() / 2;
  auto v1_filled = v1;
  v1_filled.clear();
...
float dot_product__decrypt__result0(CryptoContextT cc, CiphertextT ct, PrivateKeyT sk) {
  PlaintextT pt;
  cc->Decrypt(sk, ct, &pt);
